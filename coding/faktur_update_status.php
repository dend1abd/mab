	...
	if (isset($_POST["txtTgl1"])){		
		...
		if ($aksi == 1){
			for ($i=1; $i<=$jml; $i++){
				...
				if ($status != ""){
					$sql = "update trx_master set stFaktur='$status', ketStFaktur='$ket' where transaksi_kode='$faktur'";
					//echo($sql);
...
, '' ket_bayar, a.contact_code as kode_customer, c.contact_name as nama_customer, c.alamat, d.total_retur as jml_retur, e.contact_name as nama_sales, a.kode_wilayah, f.reff as status_faktur, a.stFaktur, a.ketStFaktur 
