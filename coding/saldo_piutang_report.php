<?php 
	...
	cekSession();
	$pageTitle = "Laporan Saldo Piutang";
	...
	//$sql = "select a.contact_code, a.contact_name, a.alamat, kota, telp, fax, email, hubungi_nama, hubungi_telp, sum(jml_in) - sum(jml_out) piutang from mst_contact a inner join trx_invoice b on a.contact_code = b.contact_code ";
	//$sql = $sql . "where a.contact_tipe = 3 and b.tgl_transaksi <= '$tgl' ";
	
	$sql = "
	select 
a.contact_code, a.contact_name, a.alamat, b.reff as kota, telp, fax, email, hubungi_nama, hubungi_telp
, ifnull(sjual.total,0) - ifnull(sretur.total,0) - ifnull(sbayar.total,0) as piutang
from mst_contact a ";
...
where transaksi_tipe in (6, 7) and transaksi_tgl <= '$tgl'
...
(
select contact_code, sum(ifnull(total,0)) as total  from trx_master 	
where transaksi_tipe in (8) and transaksi_tgl <= '$tgl'
group by contact_code
...
$sql = $sql . " where a.contact_tipe=3	and (ifnull(sjual.total,0) - ifnull(sretur.total,0) - ifnull(sbayar.total,0)) <> 0 ";
