<?php 
	...
	cekSession();
	$pageTitle = "Laporan Mutasi Hutang Detail";
	
	$oDB = new clsDataAccess($hostDB, $userDB, $passDB, $nameDB);	 
	$tgl1 = trim($_GET["tgl1"]);
	$tgl2 = trim($_GET["tgl2"]); 
	$supplier= trim($_GET["kode"]); 	 
?>
...
where a.contact_tipe=2 and a.contact_code='$supplier' ";
...
from trx_master where transaksi_tipe in (2, 3, 4)
union 
...
from trx_master a inner join trx_bayar b on a.transaksi_kode = b.transaksi_kode
where a.transaksi_tipe= 17
)b on a.contact_code = b.contact_code
where a.contact_tipe=2 and  a.contact_code='$supplier' and transaksi_tgl between '$tgl1' and '$tgl2' order by transaksi_tgl "; 
