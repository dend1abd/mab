<?php 
	...
	cekSession();
	$pageTitle = "Laporan Pengiriman Barang";
	...
	$sqlCmd = "SELECT contact_code, CONCAT(contact_name,', ', left(ifnull(alamat,''), 30)) FROM mst_contact where contact_tipe = 3 order by contact_name"; 
    $rsCustomer = $oDB->ExecuteReader($sqlCmd); 
	
	$sqlCmd = "SELECT product_code, CONCAT(product_code,' - ', left(ifnull(product_name,''), 30)) FROM mst_product"; 
    $rsProduct = $oDB->ExecuteReader($sqlCmd); 
	...
	$sql = "select a.transaksi_tgl, a.transaksi_kode, e.contact_code, e.sales_code, b.contact_name as customer, c.contact_name as sales, d.*, a.kode_divisi, a.no_reff, a.tgl_reff from trx_master a ";
	$sql = $sql . "inner join trx_detail d on a.transaksi_kode = d.transaksi_kode ";
	$sql = $sql . "inner join trx_master e on d.no_order = e.transaksi_kode ";
	$sql = $sql . "left join mst_contact b on e.contact_code=b.contact_code ";
	$sql = $sql . "left join mst_contact c on e.sales_code=c.contact_code ";
	$sql = $sql . "where a.transaksi_tipe in (19) ";
	...
	if ($contact_code != ""){
		$sql = $sql . "and e.contact_code = '$contact_code' ";  
	}
	
	if ($sales_code != ""){
		$sql = $sql . "and e.sales_code = '$sales_code' ";  
	}
                    ...
                    <td valign="top">
                    	<table>
                            <tr class='font10black'>
                                ...
                                <td>:</td>
                                <td nowrap="nowrap">
								<?php  echo getComboBox(1, "txtproduct_code", $product_code, $rsProduct, ""); ?> 
                                </td>
                            </tr>
                            <tr class='font10black'>
                                <td>Customer</td>
                                <td>:</td>
                                <td>
                                <?php  echo getAutoComplete(1, "txtcontact_code", $contact_code, 50, $rsCustomer, ""); ?> 
                                </td>
                            </tr>
                            <tr class='font10black'>
                                <td>Sales</td>
