<?php 
	...
	$sql = "select MenuKode, MenuParent, MenuName, MenuLink from mst_menu";
		...
		<td>		
			<table width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#d29fec" align="left">
				<tr class="contentTitleTable">
					<td>No</td> 
					<td>Kode</td><td>Parent</td><td>Name</td><td>Link</td>
					<td>&nbsp;</td>
				</tr>
				<?php
				if ($numRows == 0)
				...
				else
				{
					...
					while ($data = mysql_fetch_array($rs)) 
					{
						...
						<tr class='font10black' bgcolor='#ffffff'>
							<td><?php echo $i; ?></td>
							<td><?php echo $data[0] ?> </td><td><?php echo $data[1] ?> </td><td><?php echo $data[2] ?> </td><td><?php echo $data[3] ?> </td>
							<td>
							<a href="menu_entry.php?op=2&ID=<?php echo $data[0]?>">Edit</a> 
							|
							<a href="menu_entry.php?op=3&ID=<?php echo $data[0]?>">Delete</a> 
							|
							<a href="menu_entry.php?op=4&ID=<?php echo $data[0]?>">View</a> 
							</td>
						</tr>	
						<?php
					}
