<?php
	...
	if (isset($_POST["txtOp"]))
	{
        ...
        if ($_SESSION["op"] == "1" || $_SESSION["op"] == "2")
        {
			...
			$_SESSION["hutang"] = $_POST["txthutang"];
	...
	else
	{
		...
		if (isset($_GET["supplier"]))
			$supplier = retrieveS($_GET["supplier"]);
		else
			$supplier = "";
		...
		if ($_SESSION["op"] == "1")
		{
			...
			$_SESSION["contact_code"] = $supplier;
			$_SESSION["divisi"] = $divisi;
            $_SESSION["btnLabel"] = "Save Data";
            $_SESSION["lblTitle"] = "New Data Kontrabon Hutang";
        }
        elseif($_SESSION["op"] == "2" || $_SESSION["op"]=="3" || $_SESSION["op"]=="4")
        {
			...
			if ($_SESSION["op"] == "2")
			{
                ...
                $_SESSION["lblTitle"]  = "Edit Data Kontrabon Hutang";
			}
			else
			{
				$_SESSION["modeView"] = "2";
                if ($_SESSION["op"] == "3")
                {
                    ...
                    $_SESSION["lblTitle"]  = "Delete Data Kontrabon Hutang";
                }
                else
                    $_SESSION["lblTitle"]  = "View Data Kontrabon Hutang";
			}
        }
        else    
            eror("invalid op");
...
{	
    ...
    { 
		...
		$array2d[] = array("total", $_SESSION["hutang"], 0); 	
	...
	if ($_SESSION["op"] != "3")
		header("location:kontrabon_hutang_cetak.php?kode=" .$_SESSION["ID"]);
	else
		header("location:global_notification.php?from=kontrabon_hutang" .$tambahanUrl. "&strMsg=" . htmlspecialchars($strTemp));
...
{	
	...
	if ($_SESSION["op"] == "1"){
		$sql = "select no_invoice, tgl_invoice, sum(jml_in) as jml_invoice, sum(jml_out) as telah_bayar, sum(jml_in) - sum(jml_out) as hutang from trx_invoice ";
		$sql = $sql . "where contact_code='$supplier' ";
...
,(ifnull(total,0)  - ifnull(bayar,0)- ifnull(b.paid,0)) as hutang
...
(
	...
	where a.transaksi_tipe in (17) 
	group by a.contact_code, b.no_invoice
)b on a.contact_code = b.contact_code and a.transaksi_kode = b.no_invoice
where a.transaksi_tipe in (2, 3, 4) and a.contact_code ='$supplier' and a.kode_divisi ='$divisi' and (ifnull(total,0)  - ifnull(bayar,0)- ifnull(b.paid,0)) <> 0 order by transaksi_tgl, transaksi_kode";
	
	}
	else{
		$sql = "SELECT no_invoice, tgl_invoice, jml_invoice, telah_bayar, jml_hutang as hutang FROM trx_kontrabon a WHERE a.transaksi_kode ='" .$_SESSION["ID"] . "'";
	}
    ...
    $sqlCmd = "SELECT contact_code, CONCAT(contact_code,' - ', contact_name) FROM mst_contact where contact_tipe in (2)"; 
...
function frmCetak(){
	...
	//alert(kode );
	window.open("kontrabon_hutang_cetak.php?kode="+kode, "cetak_jurnal_" + kode ,  "height=360, width=640, location=0,menubar=0,scrollbars=1,resizable=0, modal=yes");

}

function frmDaftarhutang(){
}
				...
				<td  valign="top"> <!-- kanan-->
					<table width="100%">
						<tr class="font10black">
						  ...
						  <td width="1%">:</td> 
						  <td> <?php  
						  echo getComboBox(2, "txtcontact_code", $_SESSION["contact_code"], $rssupplier, ""); 
				...
				<td colspan=2 valign="top">  
					<table width="100%"  cellspacing="1" bgcolor="silver" id="tblDetail" >  
						...
						<tr class="contentTitleTable" align="center"> 
							...
							<td style="height: 22px">hutang</td>
						</tr> 
						...
						<?php
							...
							else
							{
								...
								while ($datadetail = mysql_fetch_array($rsdetail)) 
								{
									...
									echo "<td align='right'>";
									//echo setNumber($datadetail["hutang"]);
									//echo getHiddenBox(1, "txtDetailjml_hutang$i", $datadetail["hutang"]);
									echo getTextBox($_SESSION["modeView"], "txtDetailjml_hutang$i", setNumber($datadetail["hutang"]), 50, 15, $clsformatIntegerReadOnly . " readonly");
									echo "</td>";
									
									echo "</tr>";
									
									$sumhutang = $sumhutang + $datadetail["hutang"];
								}
								...
								//echo "" . setNumber($sumhutang) . "";
								echo getTextBox($_SESSION["modeView"], "txtsumhutang", setNumber($sumhutang), 50, 15, $clsformatIntegerReadOnly . " readonly"); 
								echo getHiddenBox(1, 'txtJmlItem', $jmlItem);
								echo getHiddenBox(1, 'txthutang', $sumhutang);
								echo "</b></td>";
								echo "</tr>";						
							}
						?>						
