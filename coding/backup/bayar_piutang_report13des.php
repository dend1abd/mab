...
<?php
$sql = "select no_invoice, tgl_invoice, jml_bayar from trx_bayar where transaksi_kode='" .$data["transaksi_kode"]. "'";
$rsBayar = $oDB->ExecuteReader($sql);
								...
								while ($bayar = mysql_fetch_array($rsBayar)) 
								{
									if($j==0){
										...
										echo "</tr><tr class='font10black' bgcolor='#ffffff'><td colspan=10>&nbsp;</td><td align='left' >" .$bayar["no_invoice"]. "</td>";
										echo "<td align='left' >" .$bayar["tgl_invoice"]. "</td>";
										echo "<td align='right' >" .setNumber($bayar["jml_bayar"]). "</td>";
									}
									$j++;
