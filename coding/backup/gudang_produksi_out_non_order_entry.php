<?php
	...
	$_SESSION["errMsg"] 	= "";
	$_SESSION["transaksi_tipe"] ="12";
	
	$tableGen = "trx_gd_pro_out_non_order";
	$tableName = "trx_master";
	$tableGenDetail = "trx_gd_pro_out_non_order_det";
	$tableNameDetail = "trx_detail";	
	$pageTitle = "Pengeluaran Barang Hasil Produksi non Order";
	$pagePrefix = "gudang_produksi_out_non_order";
...
{	
	...
	header("location:gudang_produksi_out_non_order_cetak.php?kode=" .$_SESSION["ID"]);
...
function sumJumlah(){
	...
	{
		qty = 0;
		if (document.getElementById('txtdetailqty_' +i).value != "") { 
			qty = parseInt(formatBilangan(document.getElementById('txtdetailqty_' +i).value)); 
		}
		sum = sum + qty; 
	}     
	document.getElementById('txtdetailqty_sum').value = formatCurrency3(sum); 
}
...
function frmCetak(){
	...
	//alert(kode );
	window.open("gudang_produksi_out_non_order_cetak.php?kode="+kode, "sales_order_" + kode ,  "height=360, width=640, location=0,menubar=0,scrollbars=1,resizable=0, modal=yes");
...
function frmCetakSJ(){
	kode = document.input.txttransaksi_kode.value;
	//alert(kode );
	window.open("sales_surat_jalan.php?kode="+kode,  "height=360, width=640, location=0,menubar=0,scrollbars=1,resizable=0, modal=yes");
