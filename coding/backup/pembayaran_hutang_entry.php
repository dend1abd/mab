<?php
	...
	else
	{
		...
		if ($_SESSION["op"] == "1")
		{
            ...
            $_SESSION["lblTitle"] = "New Data Pembayaran Hutang";
        }
        elseif($_SESSION["op"] == "2" || $_SESSION["op"]=="3" || $_SESSION["op"]=="4")
        {
			...
			if ($_SESSION["op"] == "2")
			{
                ...
                $_SESSION["lblTitle"]  = "Edit Data Pembayaran Hutang";
			}
			else
			{
				$_SESSION["modeView"] = "2";
                if ($_SESSION["op"] == "3")
                {
                    ...
                    $_SESSION["lblTitle"]  = "Delete Data Pembayaran Hutang";
                }
                else
                    $_SESSION["lblTitle"]  = "View Data Pembayaran Hutang";
			}
        }
        else    
            eror("invalid op");
...
{	
	...
	//include "posting_hutang_out.php";
	//header("location:global_notification.php?from=pembayaran_piutang" .$tambahanUrl. "&strMsg=" . htmlspecialchars($strTemp));
	
	if ($_SESSION["op"] != "3")
		header("location:pembayaran_hutang_cara_bayar_entry.php?op=" .$_SESSION["op"]. "&kode=" . $_SESSION["ID"]);
	else
		header("location:global_notification.php?from=pembayaran_hutang" .$tambahanUrl. "&strMsg=" . htmlspecialchars($strTemp));
...
{	
	...
	if ($_SESSION["op"] == "1"){
		...
		$sqlCmd = $sqlCmd . "where contact_code='$supplier' ";
...
where a.transaksi_tipe in (2, 3, 4) and a.contact_code ='$supplier' and (ifnull(total,0)  - ifnull(bayar,0)- ifnull(b.paid,0)) <> 0 order by transaksi_tgl, transaksi_kode";
...
function frmCetak(){
	...
	//alert(kode );
	window.open("pembayaran_hutang_cetak.php?kode="+kode, "cetak_jurnal_" + kode ,  "height=360, width=640, location=0,menubar=0,scrollbars=1,resizable=0, modal=yes");
