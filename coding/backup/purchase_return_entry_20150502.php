<?php
	...
	$tableGenDetail = "trx_retur_beli_detail";
	$tableNameDetail = "trx_detail";	
	$pageTitle = "Retur Pembelian Barang";
	$pagePrefix = "purchase_return";
...
function AddItem()
{
	...
	for(iLoop=1; iLoop<=10; iLoop++){
		...
		el = el + "<?php $name="\" + 'txtdetailqtysize' +iLoop+ '_' +i + \""; echo getTextBox("1", $name, "", 20, 3, " readonly"); ?>";
		el = el + "</td>";
	}
	el = el + "</tr>"
	el = el + "</table></div>"
				...
				<td colspan=2 valign="top">  
                    ...
                    <?php
						if ($_SESSION["modeView"] == "1")
						{
							...
							echo "<td width=3%><input class=\"button\" type=\"button\" name=\"btnLookUp\" value=\"...\" onClick=\"lookupWindow('product_lookup.php?tipe=1', 'product_list')\" /></td>";							
							$i = 0;
							mysql_data_seek($rsGenDetail, 0);						
							while ($dataGenDetail = mysql_fetch_array($rsGenDetail)) 
							{ 	
							...
							for ($i=1;$i<=10;$i++){
								echo "<td nowrap>";
								echo getComboBox5($_SESSION["modeView"], "txtdetailsize" .$i. "_0", "", $rsSize, "");
								echo "/";
								echo getTextBox($_SESSION["modeView"], "txtdetailqtysize" .$i. "_0", "", 10, 3, $clsformatInteger); 
								echo "</td>";	
							}
