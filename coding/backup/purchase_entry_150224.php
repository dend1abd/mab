<?php
	...
	else
	{
		...
		if ($_SESSION["op"] == "1")
		{
            ...
            $_SESSION["btnLabel"] = "Save Data Pembelian";
            $_SESSION["lblTitle"] = "New Data Pembelian";
        }
        elseif($_SESSION["op"] == "2" || $_SESSION["op"]=="3" || $_SESSION["op"]=="4")
        {
			...
			{
				$_SESSION["modeView"] = "1";
                $_SESSION["btnLabel"] = "Update Data Pembelian";
                $_SESSION["lblTitle"]  = "Edit Data Pembelian";
			}
			else
			{
				$_SESSION["modeView"] = "2";
                if ($_SESSION["op"] == "3")
                {
                    $_SESSION["btnLabel"] = "Delete Data Pembelian";
                    $_SESSION["lblTitle"]  = "Delete Data Pembelian";
                }
                else
                    $_SESSION["lblTitle"]  = "View Data Pembelian";
			}
        }
        else    
            eror("invalid op");
...
{	
    ...
    { 
		...
		$array2d[] = array("tgl_transaksi", $_SESSION["transaksi_tgl"], 1);
		$array2d[] = array("contact_code", $_SESSION["contact_code"], 1);	
		$array2d[] = array("jml_in", $_SESSION["total"], 0);
		$array2d[] = array("jml_invoice", $_SESSION["total"], 0);
	...
	header("location:global_notification.php?from=purchase" .$tambahanUrl. "&strMsg=" . htmlspecialchars($strTemp));
...
function frmCetak(){
	...
	//alert(kode );
	window.open("purchase_cetak.php?kode="+kode, "cetak_jurnal_" + kode ,  "height=360, width=640, location=0,menubar=0,scrollbars=1,resizable=0, modal=yes");
...
function frmSubmit(){ 
    ...
    { 
    	...
    	*/
    	...
    	if(document.input.txtno_invoice.value == "")
    	{
    		alert("no invoice masih kosong");	
    		return false; 
    	}
    	
    	if(document.input.txttgl_invoice.value == "")
    	{
    		alert("tgl invoice masih kosong");	
    		return false; 
    	}
    ...
    if (op == "1")
        msg = 'Confirm add Data Pembelian ?';
    else if (op == "2")
        msg = 'Confirm edit Data Pembelian ?';
    else if (op == "3")
        msg = 'Confirm delete Data Pembelian ?';
    else
    {
        msg = 'invalid op';
        return false;
			...
			<tr>
				<td width=50% valign="top"> <!-- kiri-->
					<table width="100%"> 
						...
						<tr class="font10black">
						  <td style="height: 23px"></td>
						  <td style="height: 23px">  No Invoice / Beli</td>
						  <td style="height: 23px">:</td>
						...
						<tr class="font10black">
						  <td style="height: 23px"></td>
						  <td style="height: 23px">  Tgl Invoice / Beli</td>
						  <td style="height: 23px">:</td>
