<?php
	...
	if (isset($_POST["txtOp"]))
	{
        ...
        {
            if ($_SESSION["op"] == "1"){
            	$_SESSION["jurnal_code"] = getKodeJurnalBank($hostDB, $userDB, $passDB, $nameDB); 
            	updateKodeJurnalBank($hostDB, $userDB, $passDB, $nameDB);  
            }
	...
	else
	{
		...
		if ($_SESSION["op"] == "1")
		{
			...
			ClearSession();
			 $_SESSION["jurnal_code"] = getKodeJurnalBank($hostDB, $userDB, $passDB, $nameDB);
			 $_SESSION["jurnal_code"] = str_replace("X",$divisi, $_SESSION["jurnal_code"]);
			$_SESSION["jurnal_date"] = date("Y-m-d");			
            $_SESSION["btnLabel"] = "Save Data Jurnal Bank";
            $_SESSION["lblTitle"] = "New Data Jurnal Bank";
        }
        elseif($_SESSION["op"] == "2" || $_SESSION["op"]=="3" || $_SESSION["op"]=="4")
        {
			...
			{
				$_SESSION["modeView"] = "1";
                $_SESSION["btnLabel"] = "Update Data Jurnal Bank";
                $_SESSION["lblTitle"]  = "Edit Data Jurnal Bank";
			}
			else
			{
				$_SESSION["modeView"] = "2";
                if ($_SESSION["op"] == "3")
                {
                    $_SESSION["btnLabel"] = "Delete Data Jurnal Bank";
                    $_SESSION["lblTitle"]  = "Delete Data Jurnal Bank";
                }
                else
                    $_SESSION["lblTitle"]  = "View Data Jurnal Bank";
			}
        }
        else    
            eror("invalid op");
...
{	
    ...
    if($_SESSION["op"] == "1" || $_SESSION["op"] == "2")
    {
		...
		for($i=1; $i<=$jmlItem; $i++){
			...
			$array2d[] = array("no_reff", $_POST["txtDetailNoDok$i"], 1);
...
{	
    ...
    $sqlCmd = "SELECT KodeReff, Reff FROM mst_reff where tipeReff =5"; 
    ...
    $sqlCmd = "SELECT perkiraan_code, perkiraan_name FROM mst_perkiraan where stac=1 and stbank=1"; 
...
function frmSubmit(){ 
    ...
    if (op == "1")
        msg = 'Confirm add Data Jurnal Bank ?';
    else if (op == "2")
        msg = 'Confirm edit Data Jurnal Bank ?';
    else if (op == "3")
        msg = 'Confirm delete Data Jurnal Bank ?';
    else
    {
        msg = 'invalid op';
        return false;
