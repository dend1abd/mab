<?php 
	...
	$sqlCmd = "SELECT product_code, product_name FROM mst_product order by product_name"; 
	...
	$sql = "select a.transaksi_tgl, a.transaksi_tipe, a.transaksi_kode, a.contact_code, a.sales_code, b.contact_name as customer, c.contact_name as sales, d.*, a.kode_divisi from trx_master a ";
	...
	$sql = $sql . "where a.transaksi_tipe in (6,7,8) ";
                    ...
                    <td valign="top">
                    	<table>
                            <tr class='font10black'>
                                ...
                                */
								echo getComboBox(1, "txtproduct_code", $product_code, $rsbarang, "");	
                                ?> 
                                </td>
                            </tr>
                            <tr class='font10black'>
				...
				else
				{
					...
					while ($data = mysql_fetch_array($rs)) 
					{
						...
						<tr class='font10black' bgcolor='#ffffff'>
							...
							if ($data["transaksi_tipe"] == 8){  
								$qty = $qty * -1;
								$sub_total = $sub_total * -1;
								$disc_amount = $disc_amount * -1;
							}
							$total = $sub_total - $disc_amount;
							?>
							<td align="right"><?php echo $qty ?> </td>
                            <td align="right"><?php echo $data["satuan"] ?> </td> 
							<td align="right"><?php echo setNumber($data["harga"]) ?> </td>
							<td align="right"><?php echo setNumber($sub_total) ?> </td>
							<td align="right"><?php echo setNumber($disc_amount) ?> </td> 
							<td align="right"><?php echo setNumber($total) ?> </td>
							<td><?php echo $data["ket_detail"] ?> </td>
							<td><?php echo $data["transaksi_tipe"] ?> </td>
						</tr>	
						<?php
						
						$sumqty = $sumqty + $qty;
						$sumsub_total = $sumsub_total + $sub_total; 
						$sumdisc_amount = $sumdisc_amount + $disc_amount; 
