<?php
	...
	if (isset($_POST["txtOp"]))
	{
        ...
        if ($_SESSION["op"] == "1" || $_SESSION["op"] == "2")
        {
			...
			$_SESSION["no_invoice"] = $_POST["txtno_invoice"];
			$_SESSION["tgl_invoice"] = $_POST["txttgl_invoice"];
	...
	else
	{
		...
		if ($_SESSION["op"] == "1")
		{
			...
			$_SESSION["no_invoice"] = $_SESSION["transaksi_kode"];
			$_SESSION["tgl_invoice"] = $_SESSION["transaksi_tgl"];				
            $_SESSION["btnLabel"] = "Save Data Penjualan";
            $_SESSION["lblTitle"] = "New Data Penjualan";
        }
        elseif($_SESSION["op"] == "2" || $_SESSION["op"]=="3" || $_SESSION["op"]=="4")
        {
	        ...
	        $sqlCmd = "SELECT transaksi_kode, transaksi_tipe, transaksi_tgl, contact_code, sales_code, sub_total, sub_qty, disc_persen, disc_amount, ppn_persen, ppn_amount, total, bayar, sisa, keterangan, no_invoice, tgl_invoice, no_reff, tgl_reff FROM trx_master a WHERE a.transaksi_kode ='" .$_SESSION["ID"] . "'";
			...
			if($numRows >0){				
				...
				$_SESSION["no_invoice"] = $data["no_invoice"]; 
				$_SESSION["tgl_invoice"] = $data["tgl_invoice"];
			...
			{
				$_SESSION["modeView"] = "1";
                $_SESSION["btnLabel"] = "Update Data Penjualan";
                $_SESSION["lblTitle"]  = "Edit Data Penjualan";
			}
			else
			{
				$_SESSION["modeView"] = "2";
                if ($_SESSION["op"] == "3")
                {
                    $_SESSION["btnLabel"] = "Delete Data Penjualan";
                    $_SESSION["lblTitle"]  = "Delete Data Penjualan";
                }
                else
                    $_SESSION["lblTitle"]  = "View Data Penjualan";
			}
        }
        else    
            eror("invalid op");
...
{	
    ...
    { 
		...
		$array2d[] = array("no_invoice", $_SESSION["no_invoice"], 1);
		$array2d[] = array("tgl_invoice", $_SESSION["tgl_invoice"], 1);
		...
		$array2d[] = array("tgl_transaksi", $_SESSION["transaksi_tgl"], 1);
		$array2d[] = array("contact_code", $_SESSION["contact_code"], 1);
		$array2d[] = array("jml_in", $_SESSION["total"], 0); //piutang in
		$array2d[] = array("jml_out","0", 0); //piutang out
		...
		if ($_SESSION["bayar"] != "0" && $_SESSION["bayar"] != "0.00" && $_SESSION["bayar"] != ""){
			...
			$array2d[] = array("jml_out",$_SESSION["bayar"], 0);
			$sqlCmd = sqlInsert("trx_invoice", $array2d); 
			$oDB->ExecuteNonQuery($sqlCmd);
		} 
		//update saldo
	...
	header("location:global_notification.php?from=sales" .$tambahanUrl. "&strMsg=" . htmlspecialchars($strTemp));
...
function frmCetak(){
	...
	//alert(kode );
	window.open("sales_cetak.php?kode="+kode, "cetak_jurnal_" + kode ,  "height=360, width=640, location=0,menubar=0,scrollbars=1,resizable=0, modal=yes");
...
function frmSubmit(){ 
    ...
    if (op == "1")
        msg = 'Confirm add Data Penjualan ?';
    else if (op == "2")
        msg = 'Confirm edit Data Penjualan ?';
    else if (op == "3")
        msg = 'Confirm delete Data Penjualan ?';
    else
    {
        msg = 'invalid op';
        return false;
			...
			<tr>
				<td width=50%> <!-- kiri-->
					<table width="100%"> 
						...
						<tr class="font10black">
						  <td style="height: 23px"></td>
						  <td style="height: 23px">  No Invoice</td>
						  <td style="height: 23px">:</td>
						  <td style="height: 23px"> <?php  echo getTextBox($_SESSION["modeView"], "txtno_invoice", $_SESSION["no_invoice"], 50, 20, ""); ?> </td>
						</tr >
						
						<tr class="font10black">
						  <td style="height: 23px"></td>
						  <td style="height: 23px">  Tgl Invoice</td>
						  <td style="height: 23px">:</td>
						  <td style="height: 23px"> <?php echo getDatePic($_SESSION["modeView"], "txttgl_invoice", $_SESSION["tgl_invoice"], ""); ?> </td>
						</tr >
				...
				<td valign="top"> <!-- kanan-->
					<table width="100%">
						...
						<tr class="font10black">
						  <td style="height: 23px"></td>
						  <td style="height: 23px">  No Order</td>
						  <td style="height: 23px">:</td>
						...
						<tr class="font10black">
						  <td style="height: 23px"></td>
						  <td style="height: 23px">  Tgl Order</td>
						  <td style="height: 23px">:</td>
				...
				<td colspan=2 valign="top">  
					...
					?>
						<tr bgcolor="white"  align='center'>
							<td align="right">
							<input class="button" type="button" name="btnLookUp" value="..." onClick="lookupWindow('product_lookup.php?tipe=2&elid1=txtDetailproduct_code0&elid2=txtDetailproduct_name0&elid3=txtDetailharga_beli0', 'product_list')" />
							</td>
							<td nowrap><?php 
			...
			<tr>
				...
				<td > <!-- kanan-->
					<table width="100%"> 
						...
						<tr class="font10black"> 
						  <td width="3%" style="height: 23px"></td>
						  <td width="30%" style="height: 23px">Hutang</td>
						  <td width="1%" style="height: 23px">:</td>
