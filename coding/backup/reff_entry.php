<?php
	...
	if (isset($_POST["txtOp"]))
	{
        ...
        {
            $_SESSION["tipeReff"] = $_POST["txttipeReff"];
...
$_SESSION["Reff"] = $_POST["txtReff"];
	...
	else
	{
		...
		if ($_SESSION["op"] == "1")
		{
            ...
            $_SESSION["btnLabel"] = "Save Data reff";
            $_SESSION["lblTitle"] = "New Data reff";
        }
...
$_SESSION["Reff"] = $data[3];
			...
			{
				$_SESSION["modeView"] = "1";
                $_SESSION["btnLabel"] = "Update Data reff";
                $_SESSION["lblTitle"]  = "Edit Data reff";
			}
			else
			{
				$_SESSION["modeView"] = "2";
                if ($_SESSION["op"] == "3")
                {
                    $_SESSION["btnLabel"] = "Delete Data reff";
                    $_SESSION["lblTitle"]  = "Delete Data reff";
                }
                else
                    $_SESSION["lblTitle"]  = "View Data reff";
			}
        }
        else    
            eror("invalid op");
...
{	
    if($_SESSION["op"] == "1" || $_SESSION["op"] == "2")
    {
		...
		$array2d[] = array("tipeReff", $_SESSION["tipeReff"], 0);
...
$array2d[] = array("Reff", $_SESSION["Reff"], 1);
	...
	header('location:global_notification.php?from=reff&strMsg=' . htmlspecialchars($strTemp));
...
function frmSubmit(){ 
    ...
    if (op == "1")
        msg = 'Confirm add data reff ?';
    else if (op == "2")
        msg = 'Confirm edit data reff ?';
    else if (op == "3")
        msg = 'Confirm delete data reff ?';
    else
    {
        msg = 'invalid op';
        return false;
...
<table width="70%" cellpadding="0" cellspacing="1" bgcolor="navy" align="center">
	...
	<td height="250" valign="top" align="left">
		...
		<table width="100%">
          ...
          <td>:</td>
          <td> 
          <?php  echo getComboBox($_SESSION["modeView"], "txttipeReff", $_SESSION["tipeReff"], $rsTipeReff, ""); ?>
          </td>
</tr >
<tr class="font10black">
          <td style="height: 23px"></td>
          <td style="height: 23px">  KodeReff</td>
          <td style="height: 23px">:</td>
