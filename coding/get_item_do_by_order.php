...
inner join mst_contact f on a.contact_code=f.contact_code
left join
(
select sum(IFNULL(d.qty, 0)) as d_qty,
d.no_order, d.product_code, d.kode_cust from trx_master c 
inner join trx_detail d on c.transaksi_kode=d.transaksi_kode 
where c.transaksi_tipe= 19
group by no_order, product_code, kode_cust 
)abc 
on a.transaksi_kode = abc.no_order and b.product_code = abc.product_code and a.contact_code = abc.kode_cust 
where a.transaksi_tipe=5 and a.kode_divisi='$divisi'  
...
";
	...
	echo "<td>Ket</td>";
	echo "<td>No Order</td>";
	echo "<td>Nama Cust</td>";
	echo "<td>Kode Cust</td>";
	echo "</tr>";		
	...
	else
	{
		...
		while ($dataOrder = mysql_fetch_array($rsOrder)) 
		{
			...
			echo "<td>";
			echo getTextBox("1", "txtdetailnama_cust_$i", $dataOrder["contact_name"], 20, 20,  $clsReadOnly . "  readonly"); 
			echo "</td>";		
			
			echo "<td>";
			echo getTextBox("1", "txtdetailkode_cust_$i", $dataOrder["contact_code"], 12, 12,  $clsReadOnly . "  readonly"); 
			echo "</td>";		
