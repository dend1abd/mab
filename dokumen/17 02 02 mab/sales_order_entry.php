<?php
	...
	$tableGen = "trx_order_jual";
	$tableName = "trx_master";
	$tableGenDetail = "trx_order_jual_detail";
	$tableNameDetail = "trx_detail";	
	$pageTitle = "Sales Order";
	$pagePrefix = "sales_order";
	...
	else
	{
		...
		if ($_SESSION["op"] == "1")
		{
			...
			$no_order = getKodeTransaksi2($_SESSION["transaksi_tipe"], $hostDB, $userDB, $passDB, $nameDB, $divisi); 	
			//$no_order = str_replace("X",$divisi, $no_order);
...
{	
    ...
    if($_SESSION["op"] == "1" || $_SESSION["op"] == "2")
    {
		...
		$array2d[] = array("sub_qty", $_POST["txtdetailqty_sum"], 0);
		$array2d[] = array("sub_total", $_POST["txtdetailsub_total_sum"], 0);
		...
		if ($_SESSION["op"] == "2")
        {
			include "update_faktur_by_order.php";
			include "update_do_by_order.php";
		}
	...
	if ($_SESSION["op"] == "3")
		...
		header("location:sales_order_cetak.php?kode=" .$_SESSION["ID"]);
}
...
{	
	...
	$sql = "select contact_code, sales_code, kode_wilayah from mst_contact where contact_tipe = 3 order by contact_name"; 
    $rsAbc = $oDB->ExecuteReader($sql); 
...
include "gen_ValidasiMandatory.php";
	...
	if(confirm(msg)){ 
		if (op == "1" || op == "2"){
			...
			if (op == "2") document.getElementById("btCetak").disabled = true;
...
var arrCustCode = new Array();
var arrCustWilayah = new Array();
var arrCustSales = new Array();

<?php 
	mysql_data_seek($rsAbc , 0);
	$iLoop = 0;
	while ($row = mysql_fetch_array($rsAbc)) {	
		echo "arrCustCode[$iLoop]='$row[0]';";
		echo "arrCustSales[$iLoop]='$row[1]';";
		echo "arrCustWilayah[$iLoop]='$row[2]';";		
		$iLoop++;	
	}
?>
...
function isiSales(index){
	//alert(arrCustSales[index]);
	if(index > 0){
		document.getElementById("txtsales_code").value = arrCustSales[index-1];
		document.getElementById("txtkode_wilayah").value = arrCustWilayah[index-1];	
	}
	else{
		document.getElementById("txtsales_code").value = "";
		document.getElementById("txtkode_wilayah").value = "";	
	}
}
