<?php
	...
	else
	{
        ...
        elseif($_SESSION["op"] == "2" || $_SESSION["op"]=="3" || $_SESSION["op"]=="4")
        {
	        ...
	        //$sqlCmd = "SELECT transaksi_kode, transaksi_tipe, transaksi_tgl, contact_code, bayar, keterangan, JmlCaraBayar, biaya_lain, potongan_lain, kode_divisi FROM trx_master a WHERE a.transaksi_kode ='" .$_SESSION["ID"] . "'";
			...
			if($numRows >0){				
				...
				$_SESSION["biaya_lain"] = $data["biaya_lain"]; 			
				$_SESSION["potongan_lain"] = $data["potongan_lain"]; 			
			}
			else
				eror("Data Kosong");
...
{	
    ...
    { 
		...
		for($i=1; $i<=$jmlItem; $i++){
			...
			$sql = "update trx_master set stFaktur=3 where transaksi_tipe in (6,7) and transaksi_kode='" . retrieveS($_POST["txtDetailno_invoice$i"]) ."'";
			//eror($sql);
...
where a.transaksi_tipe in (6, 7, 8) and a.contact_code ='$customer' and a.kode_divisi ='$divisi' and (ifnull(total,0)  - ifnull(bayar,0)- ifnull(b.paid,0)) <> 0 order by transaksi_tgl, transaksi_kode"; */
	...
	$sqlCmd = "select a.transaksi_kode as no_invoice, a.transaksi_tgl as tgl_invoice, ifnull(total,0) as jml_invoice, (ifnull(bayar,0)+ ifnull(b.paid,0)) as telah_bayar
,(ifnull(total,0)  - ifnull(bayar,0)- ifnull(b.paid,0)- ifnull(d.total_retur,0)) as jml_hutang
, '' ket_bayar, a.contact_code, c.contact_name, (-1 * d.total_retur) as total_retur, d.contact_name as sales_name, a.kode_wilayah
from trx_master a
left join 
(
	select a.contact_code, b.no_invoice, sum(ifnull(jml_bayar,0)) as paid
...
and a.contact_code ='$customer' and a.kode_divisi ='$divisi' ";

	}
	
	else{
		$sqlCmd = "SELECT no_invoice, tgl_invoice, jml_invoice, jml_retur as total_retur, telah_bayar, jml_hutang, jml_bayar, ket_bayar FROM trx_bayar a WHERE a.transaksi_kode ='" .$_SESSION["ID"] . "'";
	}
	//eror($sqlCmd); 
					...
					<table width="100%"  cellspacing="1" bgcolor="silver" id="tblDetail" >  
						...
						<?php
							...
							while ($datadetail = mysql_fetch_array($rsdetail)) 
							{
								...
								$jml_hutang = $datadetail["jml_hutang"];
								
								$jml_retur = $datadetail["total_retur"];
								
								if ($_SESSION["op"] == "1") 
									$jml_bayar = $datadetail["jml_hutang"];
								else
								...
								//echo setNumber($datadetail["jml_invoice"]);
								//echo getHiddenBox(1, "txtDetailjml_invoice$i", $datadetail["jml_invoice"]); 
								echo getTextBox($_SESSION["modeView"], "txtDetailjml_retur$i", setNumber($jml_retur), 50, 15, $clsformatIntegerReadOnly . " readonly");
								echo "</td>";								
								...
								echo "<td align='right'>";
								echo getTextBox($_SESSION["modeView"], "txtDetailjml_bayar$i", setNumber($jml_bayar), 50, 15, $clsformatInteger . "");
								...
								$sumJmlHutang = $sumJmlHutang + $jml_hutang;
								$sumTotal = $sumTotal + $jml_bayar;								
							}
